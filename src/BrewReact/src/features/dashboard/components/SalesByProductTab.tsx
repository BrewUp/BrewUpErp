...
export function SalesByProductTab({ refreshToken }: SalesByProductTabProps) {
  const { data, page, totalRecords, pageSize, isLoading, error, goToNextPage, goToPrevPage } =
    useSalesByProduct(refreshToken)
  ...
  return (
    <div>
      <table className="dashboard-table" aria-label="Sales by Product">
        <thead>
          <tr>
            <th>Product Name</th>
            <th>Year</th>
